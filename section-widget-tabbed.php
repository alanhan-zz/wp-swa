        ...
        if(olt_checklist_conditions_check($instance['conditions'])) {
			...
			if ($instance['accordion']) {
				foreach($instance['tabs'] as $id => $tab) {
					$list .= "<h3><a href=\"#{$widget_id}-tab-{$id}\">{$tab['title']}</a></h3><div id=\"{$widget_id}-tab-{$id}\">".do_shortcode($tab['body']).'</div>';
				}
			} else {
				foreach($instance['tabs'] as $id => $tab) {
...
function tabbed_section_widget_load_scripts() {
    ...
    if(is_admin() ) {
        ...
        if(is_active_widget(false,false,'section-tabbed')) {    
            ...
            wp_enqueue_script('section-widget', plugins_url('section-widget/section-widget.js'), array('jquery','jquery-ui-tabs','jquery-ui-accordion'));
        }
    }
}
